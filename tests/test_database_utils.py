...
def test_nn_out_of_range():
    """ Test if a nearest node can be found with coordinates out of range """
    nn = nearest_node(credentials, lat=100, lon=-200, meters=100)

    # make sure we get the right return type
    assert(type(nn) == tuple)

    # assert we have not found a point
    assert(len(nn) == 0)
...
def test_nodes_in_radius():
    ...
    sample_lat, sample_lon = 42.299944, -72.588062
    nodes = nodes_within_radius(credentials, lat=sample_lat, lon=sample_lon, radius=.05)
